package jp.co.biglobe.isp.mobile.voiceoption.api.form;

import jp.co.biglobe.lib.publication.validation.EnumForApiValue;
import jp.co.biglobe.isp.mobile.voiceoption.domain.mnpout.MnpOutCancelReason;
import jp.co.biglobe.lib.publication.form.EnumApiValueConverter;
import jp.co.biglobe.lib.publication.form.FormToValueObject;
import lombok.AllArgsConstructor;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.ToString;
import org.hibernate.validator.constraints.NotBlank;

@ToString(includeFieldNames = false)
@EqualsAndHashCode
@AllArgsConstructor
public class MnpOutCancelReasonForm implements FormToValueObject<MnpOutCancelReason> {

    @Getter
    @NotBlank
    @EnumForApiValue(MnpOutCancelReason.class)
    private String value;

    public MnpOutCancelReason getValueObject() {
        return EnumApiValueConverter.convert(MnpOutCancelReason.class, value);
    }

}
