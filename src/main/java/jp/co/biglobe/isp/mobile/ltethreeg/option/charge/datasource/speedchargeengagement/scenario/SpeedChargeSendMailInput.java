...
@AllArgsConstructor
public class SpeedChargeSendMailInput {
    ...
    private final String mailTemplate = "BLTE_speedcharge_recept";

    /**
     * ここから埋め込みパラメーター
     */

    // スピードチャージ購入日 : YYYYMMDDhh24mi
    // (itme_free4)
    @Mapping("moji1")
    private String speedChargeOrderDate;

    // スピードチャージ購入量(MB) : 1,000 (3桁カンマ区切り)
    // (itme_free7)
    @Mapping("moji4")
    private String purchasedVolumeMB;

    // スピードチャージ購入料金　税込み : 1,080 (3桁カンマ区切り)
    // (item_free8)
    // ボリュームチャージ購入通知のメールテンプレート上には存在しない変数名
    // 消費税移行期間完了後(消費税10%になったら？)税込み表示に切り替わる
    // 切り替え時にメールテンプレートの置き換えのみで対応できるよう、税込みの値も渡しておく
    @Mapping("moji5")
    private String volumeChargeFee2;

    // スピードチャージ購入料金　税抜 : 1,000 (3桁カンマ区切り)
    // (item_free9)
    @Mapping("moji6")
    private String volumeChargeFee;
