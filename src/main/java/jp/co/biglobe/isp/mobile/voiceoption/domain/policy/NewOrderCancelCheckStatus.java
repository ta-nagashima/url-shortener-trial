...
@AllArgsConstructor
public enum NewOrderCancelCheckStatus implements BusinessStatusForApi {
    NO_MNP_IN(CheckResult.OK, "MNP転入の新規申込ではない", MnpOutDetailType.NO_MNP_IN),
    NO_MNP_OUT(CheckResult.OK, "MNP転出キャンセル/MNP転出レコードなし", MnpOutDetailType.NO_MNP_OUT),
    ...
    MNP_OUT_WAITING(CheckResult.NG, "MNP予約番号発行済で転出待ち", MnpOutDetailType.MNP_OUT_WAITING),
    MNP_OUT_DONE(CheckResult.OK, "MNP転出済", MnpOutDetailType.MNP_OUT_DONE),
    NO_VOICE_OPTION(CheckResult.NG, "音声オプション契約が申込キャンセル不可能な状態", MnpOutDetailType.NO_VOICE_OPTION);
