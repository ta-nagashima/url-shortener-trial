...
set time off
set termout off
spool new_macaddress_201312.log
...
AND (use_rights_id, event_date) IN (
	...
	WHERE event_date < TO_DATE('20131201000000', 'YYYYMMDDHH24MISS') GROUP BY use_rights_id
)
AND macaddress NOT IN (
	...
	AND event_date BETWEEN TO_DATE('20131201000000', 'YYYYMMDDHH24MISS') AND TO_DATE('20131201235959', 'YYYYMMDDHH24MISS')
)
UNION
(
	/* ②指定月に登録されたMACアドレス */
	...
	FROM (
		...
		FROM wifi_register_device_event
		WHERE event_type = 'insert'
		AND event_date BETWEEN TO_DATE('20131201000000', 'YYYYMMDDHH24MISS') AND TO_DATE('20131231235959', 'YYYYMMDDHH24MISS')
	...
	WHERE EXISTS (
		...
		FROM (
			...
			FROM wifi_register_device_event
			WHERE event_type = 'delete'
			AND event_date < TO_DATE('20140101000000', 'YYYYMMDDHH24MISS')
		) DELETED_MAC
