...
set time off
set termout off
spool new_macaddress_201311.log
...
AND (use_rights_id, event_date) IN (
	...
	WHERE event_date < TO_DATE('20131101000000', 'YYYYMMDDHH24MISS') GROUP BY use_rights_id
)
...
(
	...
	FROM (
		...
		FROM wifi_register_device_event
		WHERE event_type = 'insert'
		AND event_date BETWEEN TO_DATE('20131101000000', 'YYYYMMDDHH24MISS') AND TO_DATE('20131130235959', 'YYYYMMDDHH24MISS')
	...
	WHERE EXISTS (
		...
		FROM (
			...
			FROM wifi_register_device_event
			WHERE event_type = 'delete'
			AND event_date < TO_DATE('20131201000000', 'YYYYMMDDHH24MISS')
		) DELETED_MAC
