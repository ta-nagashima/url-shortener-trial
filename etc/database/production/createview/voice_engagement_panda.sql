CREATE OR REPLACE VIEW voice_engagement_panda
AS
SELECT 1                                    AS record_id,
       TO_CHAR( engag.voice_engagement_number ) AS primary_key1,
       CAST( '0' AS VARCHAR2( 256 ) )       AS primary_key2,
       CAST( '0' AS VARCHAR2( 256 ) )       AS primary_key3,
       CAST( '0' AS VARCHAR2( 256 ) )       AS primary_key4,
       engag.equipment_number               AS foreign_key1,
       engag.lte_three_g_engagement_number  AS foreign_key2,
       engag.user_id                        AS foreign_key3,
       ...
       engag.voice_join_code                AS item_free1,
       engag.voice_engagement_status        AS item_free2,
       engag.voice_user_order_date          AS item_free3,
       null                                 AS item_free4,
       engag.engagement_start_date          AS item_free5,
       TO_CHAR( engag.engagement_end_date_time, 'YYYYMMDDHH24MISS' ) AS item_free6,
       mnpin.voice_msisdn                   AS item_free7,
       mnpin.mnp_reservation_number         AS item_free8,
       min_p.mnp_in_full_name               AS item_free9,
       min_p.mnp_in_full_name_kana          AS item_free10,
       min_p.mnp_in_birthday                AS item_free11,
       min_p.mnp_in_gender                  AS item_free12,
       min_a.mnp_activation_due_date        AS item_free13,
       mnpot.mnp_out_msisdn                 AS item_free14,
       mot_p.mnp_out_full_name              AS item_free15,
       mot_p.mnp_out_full_name_kana         AS item_free16,
       mot_p.mnp_out_birthday               AS item_free17,
       mot_p.mnp_out_gender                 AS item_free18,
       mot_n.mnp_reservation_number         AS item_free19,
       mot_n.expire_date                    AS item_free20,
       mot_n.execution_date                 AS item_free21,
       mnpot.mnp_out_status                 AS item_free22,
       mnpot.cancel_reason                  AS item_free23,
       eng_c.cancel_user_order_date         AS item_free24,
       eng_c.cancel_reason                  AS item_free25,
       eng_d.disengage_user_order_date      AS item_free26,
       eng_d.disengage_reason               AS item_free27,
       null                                 AS item_free28,
       null                                 AS item_free29,
       null                                 AS item_free30,
       null                                 AS item_free31,
       ...
       engag.system_receipt_date_time       AS item_free_date1,
       eng_c.cancel_sys_receipt_date_time   AS item_free_date2,
       eng_d.disengage_sys_receipt_date_t   AS item_free_date3,
...
FROM voice_engagement_state engag
  LEFT OUTER JOIN voice_engagement_cancel_state eng_c ON engag.voice_engagement_number = eng_c.voice_engagement_number
  LEFT OUTER JOIN voice_engagement_disengage_s  eng_d ON engag.voice_engagement_number = eng_d.voice_engagement_number
  LEFT OUTER JOIN voice_mnp_in_state            mnpin ON engag.voice_engagement_number = mnpin.voice_engagement_number
  LEFT OUTER JOIN voice_mnp_in_personal_info_s  min_p ON engag.voice_engagement_number = min_p.voice_engagement_number
  LEFT OUTER JOIN voice_mnp_in_activation_state min_a ON engag.voice_engagement_number = min_a.voice_engagement_number
  LEFT OUTER JOIN voice_mnp_out_state           mnpot ON engag.voice_engagement_number = mnpot.voice_engagement_number
  LEFT OUTER JOIN voice_mnp_out_personal_info_s mot_p ON mnpot.voice_mnp_out_id = mot_p.voice_mnp_out_id
  LEFT OUTER JOIN voice_mnp_out_number_state    mot_n ON mnpot.voice_mnp_out_id = mot_n.voice_mnp_out_id
WHERE
  engag.voice_engagement_number In (
    SELECT voice_engagement_number FROM voice_engagement_event        WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_engagement_cancel_event WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_engagement_disengage_e  WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_mnp_in_event            WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_mnp_in_personal_info_e  WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_mnp_in_activation_event WHERE event_date >= trunc(sysdate-1,'DD')
    UNION
    SELECT voice_engagement_number FROM voice_mnp_out_event           WHERE event_date >= trunc(sysdate-1,'DD')
         OR voice_mnp_out_id IN (
           SELECT voice_mnp_out_id FROM voice_mnp_out_personal_info_e WHERE event_date >= trunc(sysdate-1,'DD')
           UNION
           SELECT voice_mnp_out_id FROM voice_mnp_out_number_event    WHERE event_date >= trunc(sysdate-1,'DD')
         )
  );
